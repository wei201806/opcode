...
export const Settings: React.FC<SettingsProps> = ({
  ...
  const loadSettings = async () => {
    try {
      ...
      setError(t('settings.errorLoadSettings'));
      setSettings({});
    } finally {
      setLoading(false);
    }
  ...
  const saveSettings = async () => {
    try {
      ...
      setToast({ message: t('settings.successSaved'), type: "success" });
    } catch (err) {
      console.error("Failed to save settings:", err);
      setError(t('settings.errorSaved'));
      setToast({ message: t('settings.errorSaved'), type: "error" });
    } finally {
      setSaving(false);
    }
  };
          ...
          <Tabs value={activeTab} onValueChange={setActiveTab} className="w-full">
            ...
            <TabsContent value="general" className="space-y-6 mt-6">
              <Card className="p-6 space-y-6">
                <div>
                  <h3 className="text-heading-4 mb-4">{t('settings.general.heading')}</h3>
                  ...
                  <div className="space-y-4">
                    ...
                    {theme === 'custom' && (
                      <div className="space-y-4 p-4 border rounded-lg bg-muted/20">
                        <h4 className="text-label">{t('settings.general.customTheme.heading')}</h4>
                        ...
                        <p className="text-caption text-muted-foreground">
                          {t('settings.general.customTheme.description')}
                        </p>
                      </div>
                    )}
                    ...
                    <div className="space-y-2">
                      <div className="flex items-center justify-between">
                        <div className="flex-1">
                          <Label htmlFor="cleanup">{t('settings.general.cleanup.label')}</Label>
                          <p className="text-caption text-muted-foreground mt-1">
                    ...
                    <div className="space-y-3">
                      ...
                      {binaryPathChanged && (
                        <p className="text-caption text-amber-600 dark:text-amber-400 flex items-center gap-1">
                          ...
                          {t('settings.general.changesPending')}
                        </p>
                      )}
                    </div>
                      ...
                      <Switch
                        ...
                        onCheckedChange={async (checked) => {
                          if (checked) {
                            ...
                            setToast({ message: t('settings.general.analytics.enabled'), type: "success" });
                          } else {
                            await analytics.disable();
                            setAnalyticsEnabled(false);
                            trackEvent.settingsChanged('analytics_enabled', false);
                            setToast({ message: t('settings.general.analytics.disabled'), type: "success" });
                          }
                        }}
                      />
                    </div>
                    ...
                    {analyticsEnabled && (
                      <div className="rounded-lg border border-border bg-muted/50 p-3">
                        <div className="flex gap-2">
                          ...
                          <div className="space-y-1">
                            <p className="text-xs font-medium text-foreground">{t('settings.general.analytics.privacyHeading')}</p>
                            <ul className="text-xs text-muted-foreground space-y-0.5">
                              <li>• {t('settings.general.analytics.privacy1')}</li>
                              <li>• {t('settings.general.analytics.privacy2')}</li>
                              <li>• {t('settings.general.analytics.privacy3')}</li>
                            </ul>
                          </div>
                        </div>
                      </div>
                    ...
                    <div className="flex items-center justify-between">
                      ...
                      <Switch
                        ...
                        onCheckedChange={(checked) => {
                          ...
                          setToast({
                            message: checked
                              ? t('settings.general.tabPersistence.enabled')
                              : t('settings.general.tabPersistence.disabled'),
                            type: "success"
                          });
                        }}
                      />
                      ...
                      <Switch
                        ...
                        onCheckedChange={async (checked) => {
                          ...
                          try {
                            ...
                            setToast({
                              message: checked
                                ? t('settings.general.startupIntro.enabled')
                                : t('settings.general.startupIntro.disabled'),
                              type: 'success'
                            });
                          } catch (e) {
                            setToast({ message: t('settings.general.startupIntro.error'), type: 'error' });
                          }
                        }}
                      />
                    </div>
            ...
            <TabsContent value="permissions" className="space-y-6">
              <Card className="p-6">
                <div className="space-y-6">
                  <div>
                    <h3 className="text-heading-4 mb-2">{t('settings.permissions.heading')}</h3>
                  ...
                  <div className="pt-2 space-y-2">
                    ...
                    <ul className="text-caption text-muted-foreground space-y-1 ml-4">
                      <li>• <code className="px-1 py-0.5 rounded bg-green-500/10 text-green-600 dark:text-green-400">Bash</code> - {t('settings.permissions.example1')}</li>
                      <li>• <code className="px-1 py-0.5 rounded bg-green-500/10 text-green-600 dark:text-green-400">Bash(npm run build)</code> - {t('settings.permissions.example2')}</li>
                      <li>• <code className="px-1 py-0.5 rounded bg-green-500/10 text-green-600 dark:text-green-400">Bash(npm run test:*)</code> - {t('settings.permissions.example3')}</li>
                      <li>• <code className="px-1 py-0.5 rounded bg-green-500/10 text-green-600 dark:text-green-400">Read(~/.zshrc)</code> - {t('settings.permissions.example4')}</li>
                      <li>• <code className="px-1 py-0.5 rounded bg-green-500/10 text-green-600 dark:text-green-400">Edit(docs/**)</code> - {t('settings.permissions.example5')}</li>
                    </ul>
                  </div>
                </div>
              </Card>
            ...
            <TabsContent value="environment" className="space-y-6">
              <Card className="p-6">
                <div className="space-y-6">
                  <div className="flex items-center justify-between">
                    <div>
                      <h3 className="text-heading-4">{t('settings.environment.heading')}</h3>
                  ...
                  <div className="pt-2 space-y-2">
                    ...
                    <ul className="text-caption text-muted-foreground space-y-1 ml-4">
                      <li>• <code className="px-1 py-0.5 rounded bg-blue-500/10 text-blue-600 dark:text-blue-400">CLAUDE_CODE_ENABLE_TELEMETRY</code> - {t('settings.environment.example1')}</li>
                      <li>• <code className="px-1 py-0.5 rounded bg-blue-500/10 text-blue-600 dark:text-blue-400">ANTHROPIC_MODEL</code> - {t('settings.environment.example2')}</li>
                      <li>• <code className="px-1 py-0.5 rounded bg-blue-500/10 text-blue-600 dark:text-blue-400">DISABLE_COST_WARNINGS</code> - {t('settings.environment.example3')}</li>
                    </ul>
                  </div>
                </div>
              </Card>
            ...
            <TabsContent value="advanced" className="space-y-6">
              <Card className="p-6">
                <div className="space-y-6">
                  <div>
                    <h3 className="text-base font-semibold mb-4">{t('settings.advanced.heading')}</h3>
            ...
            <TabsContent value="hooks" className="space-y-6">
              <Card className="p-6">
                <div className="space-y-4">
                  <div>
                    <h3 className="text-base font-semibold mb-2">{t('settings.hooks.heading')}</h3>
                    <p className="text-body-small text-muted-foreground mb-4">
                      {t('settings.hooks.description1')}
                      <code className="mx-1 px-2 py-1 bg-muted rounded text-xs">~/.claude/settings.json</code>
                      {t('settings.hooks.description2')}
                    </p>
                  </div>
